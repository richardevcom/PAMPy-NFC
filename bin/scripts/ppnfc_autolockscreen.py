...
This script is a PAM.py NFC client. It periodically requests the authentication
status of the current user from the PAM.py NFC server and works out when to lock
...
Note that unlocking the session manually with a password or with PAM.py NFC PAM
...
def main():
    ...
    while True:
        ...
        if cycle_start_tstamp > recheck_session_locked_tstamp or \
            ...
            session_locker_status = execute_command(
                command=QUERY, verbose=do_verbose)
