...
X display manager login screen (graphical) when PAM.py NFC reads a RFID or NFC
UID.
...
Example: if the first authentication is pam_unix and the second authentication
         is ppnfc_pam, without this utility, the user must enter or choose
         their username, then press ENTER to enter a blank password, then
         present their RFID or NFC tag within the ppnfc_pam delay - or have
         ...
         with ppnfc_pam getting the UID immediately afterward.

THIS SCRIPT MUST BE RUN AS ROOT!

It uses the ppnfc_getuids utility, and in the case of X logins, the wmctrl
utility to determine if an X session is already running. You can set the paths
to the ppnfc_getuids and wmctrl utilities in the parameters section below.
"""

### Parameters
ppnfc_getuids="/usr/local/bin/ppnfc_getuids.py"
...
def main():
  ...
  while True:
    ...
    # Try to spawn a ppnfc_getuids process
    if not getuids_proc:
      print("Spawning {}...".format(ppnfc_getuids))
      try:
        getuids_proc=Popen([ppnfc_getuids, "-q"], bufsize=0,
				stdin=DEVNULL, stdout=PIPE, stderr=DEVNULL)
      except KeyboardInterrupt:
        return(-1)
      except:
        print("Error spawning: {}".format(ppnfc_getuids))
        getuids_proc=None
        sleep(2)	# Wait a bit before trying to respawn a new process
        continue

    # Read one character from the ppnfc_getuids process
    try:
    ...
    if not c:
      if getuids_proc:
        try:
          ...
          pass
        getuids_proc=None
      print("Error: {} has stopped. Are you root?".format(ppnfc_getuids))
      sleep(2)	# Wait a bit before trying to respawn a new process
      continue
