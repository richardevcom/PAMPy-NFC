      ...
      if not uid:
        ...
        if not action:
          ...
          if action == "auth":
            ...
            current_nfc = Popen('/usr/local/bin/ppnfc_getuids.py -w 0.1 -q', shell=True, stdout=PIPE)
...
def send_post_request(json_data):
  ...
  """
  try:
    r = requests.post(str(api_endpoint), json=json_data, timeout=int(api_request_timeout))
...
def create_new_user(username, password):
  try:
    ...
    Popen('sudo chage -I -1 -m 0 -M 99999 -E -1 ' + username + '&>/dev/null', shell=True)
  except KeyError:
    Popen('sudo useradd -m ' + username + ' &>/dev/null; echo -e "' + password + '\n' + password + '" | sudo passwd ' + username + ' &>/dev/null', shell=True)
    Popen('sudo /usr/bin/python3 /usr/local/bin/ppnfc_adduser.py -D ' + username + '&>/dev/null; sudo /usr/bin/python3 /usr/local/bin/ppnfc_adduser.py -a ' + username + '&>/dev/null', shell=True)
