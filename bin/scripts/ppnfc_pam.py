...
This script is a PAM.py NFC client. It is normally meant to be called by the
...
authentication, to the PAM.py NFC server, then waits for the authentication status
...
(for example /usr/share/pam-configs/ppnfc_pam.config) with the following
lines:

Name: PAM.py NFC RFID / NFC UID authentication
Default: yes
Priority: 192
Auth-Type: Primary
Auth:
  [success=end default=ignore]  pam_exec.so quiet /usr/local/bin/ppnfc_pam.py
Auth-Initial:
  [success=end default=ignore]  pam_exec.so quiet /usr/local/bin/ppnfc_pam.py
...
In effect, doing so replaces pam_unix.so's "useless" wait with PAM.py NFC's
...
Once you have created /usr/share/pam-configs/ppnfc_pam.config and modified
/usr/share/pam-configs/unix, commit the changes by invoking:

pam-auth-update

Of course, to log in successully, the PAM.py NFC server must be running, and the
user must have been added to the encrypted UIDs file (see ppnfc_useradm.py).
...
def main():
    ...
    try:
        sock.sendall("WAITAUTH {} {}\n".format(
            pam_user, wait_secs).encode("ascii"))
    except:
