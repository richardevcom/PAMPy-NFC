...
read by the PAM.py NFC server as characters "typed" by a keyboard.

Without arguments, the program "types out" the UIDs read by the PAM.py NFC server
...
- All UIDs on one line, separated by spaces:
    ...
    ppnfc_keyboard_wedge.py -s ' '

- "UID=" before the value, semicolon after the value, one per line:

    ppnfc_keyboard_wedge.py -p UID= -s $';\n'

This program is a PAM.py NFC client. It requires the PAM.py NFC server to read the
                    ...
                    for c in re.sub("\n", "\r", args.prefix) + uid + \
                        ...
                        for m in modifiers_to_ecodes:
                            if ecode & m:
                                ...
                                modseq_keyup = [
                                    [modifiers_to_ecodes[m], KEY_UP]] + modseq_keyup
